...
- [x] 2026-09-08 — Move planning docs into `docs/`, add `.gitignore`, add `src/_headers`, and document Cloudflare Pages dashboard deploy (no Wrangler).
...
- `src/index.html` is 1,568 lines: CSS + `AiStrip` core IIFE + presentation IIFE in one file.
...
- Publishing the git root 404s `/` because `index.html` lives under `src/`; Pages must publish `dist/` after `npm run build`.
