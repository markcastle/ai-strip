...
- **Deploy uses `dist/`.** `npm run build` copies `src/` to `dist/`. `dist/` is committed so Cloudflare Pages can publish it even when the dashboard build command is left blank. Dashboard: output directory `dist`. Do not publish the repo root.
...
webapp/
  ...
  dist/                generated by `npm run build`; committed so Pages can publish it
  tests/build.test.js
  README.md
  LICENSE
  .gitignore
...
Cloudflare Pages (dashboard, no Wrangler): framework None, build command blank or `npm run build`, build output directory `dist`, production branch `master`.
